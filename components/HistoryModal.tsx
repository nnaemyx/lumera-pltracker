...
export default function HistoryModal({ isOpen, onClose }: HistoryModalProps) {
  ...
  const getTypeColor = (type: string) => {
    ...
    if (type.includes("Delegate") && !type.includes("Undelegate")) return "text-green-400 bg-green-950/30 border border-green-500/30";
    ...
    return "text-blue-400 bg-blue-950/30 border border-blue-500/30";
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-4">
      <div className="bg-[#111118] rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col border-2 border-[#1e1e2e]">
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {transactions.map((tx, index) => (
                <div
                  key={tx.hash || index}
                  className="bg-[#0a0a0f] rounded-xl p-4 hover:shadow-lg transition-shadow border border-[#1e1e2e]"
                >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {tx.to && (
                          <p className="text-xs text-gray-500 mb-1">
                            To: <span className="font-mono text-gray-400">{shortenAddress(tx.to)}</span>
                          </p>
                        )}
                        {tx.validatorAddress && (
                          <p className="text-xs text-gray-500 mb-1">
                            Validator: <span className="font-mono text-gray-400">{shortenAddress(tx.validatorAddress)}</span>
                          </p>
                        )}
                        {tx.memo && (
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500 mt-2">
                          ...
                          <a
                            ...
                            className="flex items-center gap-1 text-[#00ff88] hover:underline font-semibold"
                          >
